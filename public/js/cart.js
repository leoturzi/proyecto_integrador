window.addEventListener('load', (e) => {
    ...
    let totalAmountOutput = document.querySelector('.cart-subtotal__total-amount')// tabla orders --> amount
        ...
        cart.forEach((item, i) => {
            ...
            .then(data => {
                ...
                <div class="cart-item__desc">
                    ...
                    <div>
                    <p class="cart-item__color"> ${product.name}</p> <a class="cart-item__color-picklist">${product.color}</a>
                    <span class="cart-item__dispatch">${product.dispatch == 1 ? 'Envío Gratis' : ''}</span>
                    </div>
                    </div>
                    ...
                    <a class='item--cursor-pointer'><i class="fa-solid fa-trash-can" data-pid="${item.id}"></i></a>
                    <p class="cart-item--green"> <span class="cart-item__price">$ ${toThousand(parseFloat(product.price))}</span></p>
                    </div>
                    <p class="cart-item__total-price">$ ${toThousand(parseFloat(product.price * item.q, 2).toFixed(2))}</p>
                    </div>
    ...
    cartContainer.addEventListener('submit',(e) => {
        e.preventDefault();
        let paymentMethodInput = document.querySelector('#paymentMethod').value;
